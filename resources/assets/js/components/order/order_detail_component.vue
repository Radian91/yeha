        ...
        <div class="col-md-12">
            ...
            <div class="d-flex flex-wrap mb-4">
                ...
                <div class="ml-auto">
                    ...
                    <button class="btn btn-outline-primary mr-1" v-if="printnode_enabled == true" v-on:click="printnode_print('POS_INVOICE')" v-bind:disabled="processing == true"> <i class='fa fa-circle-notch fa-spin'  v-if="processing == true"></i> {{ $t("Print Invoice") }}</button>

                    <button class="btn btn-outline-primary" v-if="print_kot_link != '' && printnode_enabled == true" v-on:click="printnode_print('KOT')" v-bind:disabled="kot_processing == true"> <i class='fa fa-circle-notch fa-spin'  v-if="kot_processing == true"></i> {{ $t("Print KOT") }}</button>
        ...
        methods: {
           delete_order(){
            ...
            printnode_print(type){
                ...
                (type == 'POS_INVOICE')?this.processing = true:this.kot_processing = true;
                ...
                axios.post(this.printnode_api_link, formData).then((response) => {
                    if(response.data.status_code == 200) {
                        ...
                        (type == 'POS_INVOICE')?this.processing = false:this.kot_processing = false;
                    }else{
                        (type == 'POS_INVOICE')?this.processing = false:this.kot_processing = false;
                        try{
                            var error_json = JSON.parse(response.data.msg);
